...
func LogMaxAge() int {
	logMaxAge := DEFAULT_LOGMAXAGE
	flag.Visit(func(f *flag.Flag) {
		if f.Name == "log_max_age" {
...
func removeFiles(logDir string, prefix string, logMaxAge int) error {
	...
	timeToRetain := time.Now().Add(-time.Duration(int64(24*time.Hour) * int64(logMaxAge)))
